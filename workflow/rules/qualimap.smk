...
rule qualimap:
    ...
    shell:
        """
        mkdir -p qc/qualimap_ug/{wildcards.sample}_{wildcards.unit}
        qualimap bamqc -bam {input.bam} -c -nt {resources.cpus_per_task} --java-mem-size={resources.mem_mb}M -outdir qc/qualimap_ug/{wildcards.sample}_{wildcards.unit} &> {log}
        """
